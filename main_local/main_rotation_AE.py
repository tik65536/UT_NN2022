...
AE = getattr(importlib.import_module(_model,package='VAE.AE'),_class)
...
validateloader = torch.utils.data.DataLoader(validationset,batch_size=validationbatchsize)
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(trainloader):
        ...
        s,t = data
        out,mu =model(s.to(model.device))
        totalloss=reconstructionlossfunction(out,t.to(model.device))
        optimizer.zero_grad()
        totalloss.backward()
        optimizer.step()
        trainingloss += totalloss.item()
    print(f"Training Epoch : {epoch:3d} | Total loss : {(trainingloss/i):.4f} ")
    corasegain={}
    handler=[]
    model.eval()
    base=0
    ...
    for name,layer in model.FCM.named_modules():
        if isinstance(layer, torch.nn.Linear):
            base+=1
            handler.append(layer.register_forward_hook(get_output(f'featureLinear_{base}',corasegain)))
    j=0
    out=None
    t=None
    v1=None
    ...
    with torch.no_grad():
        for j, data in enumerate(validateloader):
            ...
            if(j==0):
                ...
                for h in handler:
                    h.remove()
            writer.add_histogram(f'F Histogram (first 5)',mu, epoch)
            vrloss=reconstructionlossfunction(out,t.to(model.device))
            validateloss+= vrloss.data
    ...
    with pymp.Parallel(_mp) as p:
        for k in p.range(len(layerName)):
            perviousScale=None
            perviousName='X'
            MI=None
            if(k!=0):
            ...
            if(k<len(layerName)-1):
                ...
                #S=pymp.shared.array((allScale.shape[0],allScale.shape[1]))
                S=np.zeros((allScale.shape[0],allScale.shape[1]))
                #with pymp.Parallel(2) as q:
    ...
    print(f"Validation Epoch : {epoch:3d} | Total loss : {(validateloss/j):10.8f}| Model Time :{modelendTime-start} | DataProcessing Time :{end-modelendTime}")
