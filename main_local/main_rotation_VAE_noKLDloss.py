...
tensorboardPath=f'../Tensorboard/'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+\
                f'_R{_round}_bsize{_bsize}_5e7d_feature_{_model}_noKLD_brightness2_solarize0_{_pool}_CoraseGrain'
...
for epoch in range(epochs):
    ...
    with pymp.Parallel(_mp) as p:
        for k in p.range(len(layerName)-1):
            ...
            if(k<len(layerName)-2):
                ...
                with pymp.Parallel(_mp) as q:
                    for e in q.range(allScale.shape[0]):
                        f = allScale[e]
    ...
    print(f"Validation Epoch : {epoch:3d} | Total loss : {(validateloss/j):10.8f} | Reconstruction loss: {(validationreconstructionloss/j):10.8f} | KLD loss: {(validationKLDloss/j):10.8f} | Model Time : {modelendTime-start} | DataProcessing Time : {end-modelendTime}")
