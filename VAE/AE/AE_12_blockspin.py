...
class AE(torch.nn.Module):
    def __init__(self,ek_size=2,dk_size=3,channel=3,pool='avg',device=None):
        ...
        if(pool=='avg'):
            self.encoder =  torch.nn.Sequential(
                torch.nn.Conv2d(channel, 64, 2, stride=ek_size),  #  C=64,H=102,W=188
                torch.nn.ReLU(),
                #torch.nn.AvgPool2d(3, stride=2),                       #  C=64,H=50,W=93
                torch.nn.Conv2d(64, 128, 2,stride=ek_size),  #  C=128,H=50,W=93
                torch.nn.ReLU(),
                #torch.nn.AvgPool2d(3, stride=2),                                             #  C=128,H=24,W=46
                torch.nn.Conv2d(128, 256, 2,stride=ek_size),   #  C=256,H=24,W=46
                torch.nn.ReLU(),
                #torch.nn.AvgPool2d(3, stride=2),                                           #  C=256,H=11,W=22
                torch.nn.Conv2d(256, 512, 2,stride=ek_size),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
                #torch.nn.AvgPool2d(3, stride=2),
                torch.nn.Conv2d(512, 512, 2,stride=ek_size),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
                #torch.nn.AvgPool2d(3, stride=2),
                torch.nn.Conv2d(512, 512, 2,stride=ek_size),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
