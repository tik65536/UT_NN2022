...
class AE(torch.nn.Module):
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        self.FCM = torch.nn.Linear(512*4*4,12).to(self.device)
        self.encoder=None
        ...
        if(pool=='avg'):
            self.encoder =  torch.nn.Sequential(
                ...
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(3, stride=4),                       #  C=64,H=50,W=93
                torch.nn.Conv2d(64, 256, 3,stride=1,padding=1),  #  C=128,H=50,W=93
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(3, stride=4),                                           #  C=256,H=11,W=22
            ...
            self.encoder =  torch.nn.Sequential(
               torch.nn.Conv2d(channel, 64, ek_size, stride=1,padding=self.epaddingsize),  #  C=64,H=102,W=188
                torch.nn.ReLU(),
                torch.nn.MaxPool2d(3, stride=4),                       #  C=64,H=50,W=93
                torch.nn.Conv2d(64, 256, 3,stride=1,padding=1),  #  C=128,H=50,W=93
                torch.nn.ReLU(),
                torch.nn.MaxPool2d(3, stride=4),                                           #  C=256,H=11,W=22
                torch.nn.Conv2d(256, 512, 3,stride=1,padding=1),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
        ...
        self.decoder =  torch.nn.Sequential(
            torch.nn.ConvTranspose2d(3,512,3,stride=1),        #   (64,6,6) 6        #   (64,6,6) 6
            torch.nn.ReLU(), #(6,5),
            torch.nn.ConvTranspose2d(512,256,4,stride=4),        #   (64,6,6) 6        #   (64,6,6) 6
            torch.nn.ReLU(), #(6,5)
            torch.nn.ConvTranspose2d(256,64,4,stride=4),        #   (512,13,13) 14
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64,3,4,stride=4),        #   (256,27,27) 30        #   (3,231,231) 254
        ).to(self.device)
