...
class AE(torch.nn.Module):
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        self.FCM = torch.nn.Linear(512*3*3,256).to(self.device)
        self.encoder=None
        if(pool=='avg'):
            ...
            self.encoder =  torch.nn.Sequential(
                ...
                torch.nn.Conv2d(512, 512, 3,stride=1,padding=1),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
                torch.nn.MaxPool2d(3, stride=2),  #  C=512,H=5,W=10 = 25600 - mu C[0:256] , v C[256:512]
            ).to(self.device)
        self.decoder =  torch.nn.Sequential(
            # input(16,4,4)
            torch.nn.ConvTranspose2d(16,64,3,stride=1),        #   (64,6,6) 6
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64,512,4,stride=2),        #   (512,13,13) 14
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(512,256,4,stride=2),        #   (256,27,27) 30
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(256,128,4,stride=2),        #   (128,55,55) 62
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128,64,5,stride=2),        #   (64,111,111) 127
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64,3,4,stride=2)        #   (3,231,231) 254
