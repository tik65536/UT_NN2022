...
class AE(torch.nn.Module):
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        self.FCM = torch.nn.Linear(3*3*3,12).to(self.device)
        self.encoder=None
        #self.FCV = torch.nn.Linear(512*3*3,12).to(self.device)
        if(pool=='avg'):
            self.encoder =  torch.nn.Sequential(
                torch.nn.Conv2d(channel, 3, ek_size, stride=1,padding=self.epaddingsize),  #  C=64,H=102,W=188
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(3, stride=2),                       #  C=64,H=50,W=93
                torch.nn.Conv2d(3, 3, 3,stride=1,padding=1),  #  C=128,H=50,W=93
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(3, stride=2),                                             #  C=128,H=24,W=46
                torch.nn.Conv2d(3, 3, 3,stride=1,padding=1),   #  C=256,H=24,W=46
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(3, stride=2),                                           #  C=256,H=11,W=22
                torch.nn.Conv2d(3, 3, 3,stride=1,padding=1),  #  C=512,H=11,W=22
                torch.nn.ReLU(),
    ...
    def forward(self, face):
        ...
        facefeature = facefeature.view(-1,3*3*3)
