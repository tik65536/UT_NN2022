...
class AE(torch.nn.Module):
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        self.FCM = torch.nn.Linear(32*4*4,12).to(self.device)
        self.encoder=None
        #self.FCV = torch.nn.Linear(512*3*3,12).to(self.device)
        if(pool=='avg'):
            self.encoder =  torch.nn.Sequential(
                torch.nn.Conv2d(channel, 32, 64, stride=64),  #  C=64,H=102,W=188
                torch.nn.ReLU(),              
                #self.activation
        ...
        self.decoder =  torch.nn.Sequential(
            torch.nn.ConvTranspose2d(3,32,3,stride=1),        #   (64,6,6) 6
            torch.nn.ReLU(), #(6,5)
            torch.nn.ConvTranspose2d(32,3,64,stride=64)        #   (512,13,13) 14
        ).to(self.device)
    ...
    def forward(self, face):
        ...
        facefeature = facefeature.view(-1,32*4*4)
