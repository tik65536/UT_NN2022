    ...
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        if(pool=='avg'):
            self.encoder =  torch.nn.Sequential(
                torch.nn.Conv2d(channel, 512, ek_size, stride=1,padding=self.epaddingsize),  #  C=64,H=102,W=188
                torch.nn.ReLU(),
                torch.nn.AvgPool2d(64, stride=64)                
                #self.activation
        ...
        self.decoder =  torch.nn.Sequential(
            torch.nn.ConvTranspose2d(3,512,3,stride=1),        #   (64,6,6) 6
            torch.nn.ReLU(), #(6,5)
            torch.nn.ConvTranspose2d(512,3,64,stride=64)        #   (512,13,13) 14
        ).to(self.device)
