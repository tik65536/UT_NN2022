...
class VAE(torch.nn.Module):
    def __init__(self,ek_size=3,dk_size=3,channel=3,pool='avg',device=None):
        ...
        self.FCM = torch.nn.Linear(512*3*3,16).to(self.device)
        self.FCV = torch.nn.Linear(512*3*3,16).to(self.device)
        self.encoder=None
        if(pool=='avg'):
        ...
        self.decoder =  torch.nn.Sequential(
            # input(16,4,4)
            torch.nn.ConvTranspose2d(1,64,3,stride=1),        #   (64,6,6) 6
            torch.nn.ReLU(),
